								...
								<tr>
									<td>
										<form action="" method="post" onsubmit="return test()">
											...
											<input type="number" min="0" max="${vo.error_count}" name="error_count" placeholder="수량을 적으세요." required="required" onkeyup="this.value=numberFormat(this.value, false, false, false)">
											<input type="hidden" name="pno" value="${vo.pno}"> 
											<input type="submit" value="불량 보내기" class="btn btn-danger">
										</form>
									</td>
									<td>${vo.pno}</td>
									<td>${vo.pname}</td>
